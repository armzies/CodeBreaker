...
export function PlayingPhase({ socket, gameId, isMyTurn, turnStartTime, history, myId, isFinished, winner, me, opponent, onPlayAgain, onLeave }: PlayingPhaseProps) {
  ...
  const opponentHistory = history.filter(h => h.playerId !== myId).reverse();

  return (
    <div className="w-full max-w-5xl flex flex-col lg:grid lg:grid-cols-3 gap-6 p-2 sm:p-4">
      
      {/* Main Play Area (Top on Mobile, Center on Desktop) */}
      <div className="flex flex-col items-center justify-center space-y-6 lg:col-start-2 lg:row-start-1">
      ...
      {/* History Area (Bottom on Mobile as 2 columns, Sides on Desktop) */}
      <div className="grid grid-cols-2 gap-4 lg:contents">
        
        {/* My History */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 sm:p-6 flex flex-col h-[40vh] min-h-[300px] lg:h-[600px] lg:col-start-1 lg:row-start-1">
        ...
        {/* Opponent History */}
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-4 sm:p-6 flex flex-col h-[40vh] min-h-[300px] lg:h-[600px] lg:col-start-3 lg:row-start-1">
          <h3 className="text-lg sm:text-xl font-bold text-zinc-400 mb-3 border-b border-zinc-800 pb-2">Opponent's</h3>
          <div className="flex-1 overflow-y-auto space-y-2 sm:space-y-3 pr-1 sm:pr-2 custom-scrollbar">
            <AnimatePresence>
              {opponentHistory.map((h, i) => (
                <motion.div 
                  ...
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  className="bg-zinc-950 border border-zinc-800 rounded-xl p-3 sm:p-4 flex flex-col xl:flex-row justify-between items-center text-center xl:text-left gap-1 opacity-70"
                >
                  <span className="font-mono text-xl sm:text-2xl tracking-widest text-zinc-400">{h.guess}</span>
                  <div className="flex items-center space-x-1 sm:space-x-2">
                    <span className="text-zinc-300 font-bold text-base sm:text-lg">{h.correctPositions}</span>
                    <span className="text-zinc-600 text-xs sm:text-sm uppercase tracking-wider">Correct</span>
                  </div>
                </motion.div>
              ))}
